...
> **제공**: 황룡건설(주) 안전보건기획부 · **개발**: ㈜라텔웍스 (Ratelworks Inc.) · **라이선스**: MIT

이 프로젝트는 **관련 법령과 KOSHA 가이드를 비롯한 공공데이터를 온톨로지 그래프 기반으로 제공**하여 AI Agent 에게 **안전관리 도메인의 전문성을 부여**하는 OSS MCP 서버입니다.

안전관리자·현장소장이 매일·매주·매월 작성하는 **19종 법정 안전관리 문서** (TBM·작업계획서·위험성평가·MSDS·산재조사표 등) 를 정확하고 빠르게 작성하도록 옆에서 돕습니다. **안전관리 법령 8개 본문** (산안법·시행령·시행규칙·기준규칙·중처법·중처법 시행령·위험성평가 고시·건진법 §62 영역) + **KOSHA Guide 1,039건 본문** (offline, keyless) + 부처별 분산 공공데이터 (법제처·KOSHA·고용노동부·국토부) 를 같은 온톨로지 그래프로 통합해, 작성 시점에 양식·빈칸 가이드·법령 본문 발췌·검수를 자동 제공합니다.
...
MCP 서버 설계·구현·오픈소스 유지. 공공 안전자료 접근성 개선.
