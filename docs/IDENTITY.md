...
agent-safety-oss 는 **관련 법령과 KOSHA 가이드를 비롯한 공공데이터를 온톨로지 그래프 기반으로 제공**하여 AI Agent 에게 **안전관리 도메인의 전문성을 부여**하는 OSS MCP 서버다. 안전관리자·현장소장이 매일·매주·매월 작성하는 **19종 법정 안전관리 문서** (TBM·작업계획서·위험성평가·MSDS·산재조사표 등) 를 정확하고 빠르게 작성하도록 옆에서 돕는 작성 보조 동작이 1차 가치. **작성 주체는 안전관리자, MCP 는 보조**.
...
- **KOSHA Guide 1,039건** (2026.1 재정비 후 유효본)
