...
decision 004(양방향 그래프 통합, v1.5.0)는 "LLM 이 그래프 SSoT 에서 IRI 인용 → 환각 차단"을 핵심 정신으로 기록했다. 그래프 데이터 레이어는 3중 게이트(`validate-shapes` SHACL / `audit:strict` / `verify:edge-context`)를 모두 통과하며 건강하다(2026-05-26 실측: 위반 0 / dangling 0 / cycle 0). `art:중처법시행령:4` 같은 조문 노드도 정상 존재한다.
...
3. **그래프 노드 SSoT 우선 + ref→IRI 단일화 (이번 결정)** ✅ — 그래프가 이미 권위 SSoT(SHACL·audit PASS)이므로 코드를 거기에 맞춤. decision 004 정신과 정합.
...
- decision 004 "그래프 SSoT 환각차단" 정신을 코드 레이어까지 관철.
...
- decision 004 (양방향 그래프 통합) — "그래프 SSoT 에서 IRI 인용 → 환각 차단" 정신
