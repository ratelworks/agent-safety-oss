...
interface Violation {
  category: "internal_term" | "internal_path" | "pii";
...
// ── 출력 ────────────────────────────────────────────────────
console.log("OSS hygiene 검증 (내부 용어 / 내부 경로 / PII)");
console.log(`검사 대상: ${targets.length} 파일`);
console.log();
...
for (const cat of ["internal_term", "internal_path", "pii"] as const) {
